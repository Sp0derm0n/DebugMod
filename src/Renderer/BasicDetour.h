...
class VTableDetour 
{
    public:
        ...
        bool Attach() noexcept {
            assert(!attached);
            new (&hooks) PLH::VFuncSwapHook(reinterpret_cast<uint64_t>(target), redirects, &originals);
...
template <typename T, AutoResolveREL Res = AutoResolveREL::Yes>  // Should be No for SE
class TypedDetour {
    public:
        ...
        bool Attach() noexcept {
            // @Smoothcam Note: I tried using polyhook (with Capstone and Zydis), some methods it just fails to hook, so we are stuck
            // with this for now
            assert(!attached);
            DetourTransactionBegin();
            DetourUpdateThread(GetCurrentThread());

            attached = DetourAttach(reinterpret_cast<void**>(&fnOrig), reinterpret_cast<void*>(fnDetour)) == NO_ERROR;
            if (attached) {
        ...
        T GetBase() noexcept { return fnOrig; }

        uintptr_t BaseAddr() const noexcept { return baseAddr; }
